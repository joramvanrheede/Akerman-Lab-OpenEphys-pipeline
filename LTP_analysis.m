...
LTP_expt_dir                = '/Volumes/Akermanlab/Joram/LTPtest'; % where are the data files? This script will 
...
n_per_point         = [4];              % How many repeats to average per data point; 1 = plot every data point individually
...
trace_smooth_win    = [7];              % Smoothing window for plotting PSTHs
...
close all
%%
pre_LTP_dir                 = [LTP_expt_dir filesep 'Test_12_pre'];

pre_LTP_folders           	= dir(pre_LTP_dir);
qremove                     = ismember({pre_LTP_folders.name},{'.','..','.DS_Store'});
pre_LTP_folders(qremove) 	= [];
...
expt_start_times            = [];
expt_end_times              = [];
for a = 1:length(pre_LTP_folders)
    day_folder              = [pre_LTP_dir filesep pre_LTP_folders(a).name];
    pre_files               = dir(day_folder);
    qremove               	= ismember({pre_files.name},{'.','..','.DS_Store'});
    pre_files(qremove)      = [];
    ...
    time_vect               = [];
    for b = 1:length(pre_files)
        this_file = [day_folder filesep pre_files(b).name];
...
for a = 1:4
    ...
    % set(plotaxes,'Ylim',[0 max(maxy)],'XLim',[0 max(time_vect) + time_vect(1)])
