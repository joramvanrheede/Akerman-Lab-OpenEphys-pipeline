...
spikes              = permute(spikes, [2 1 3]); % Make trials the first dimension
spikes_by_trial     = reshape(spikes,[size(spikes,1) size(spikes,2)*size(spikes,3)]);
spike_rate_counts 	= histc(spikes_by_trial,spike_rate_bins,2);
