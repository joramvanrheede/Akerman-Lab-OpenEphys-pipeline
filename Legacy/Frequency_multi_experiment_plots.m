...
save_expt_name      = '20180605_Frequency';
save_figs        	= true;
...
LEDresp_threshold   = 4;  % threshold relative to spontaneous


qpktimeplots        = false;
qrespsizeplots      = false;
qLEDplots           = false;
...
resp_measure        = 'adj'; % 'peak', 'bin' or 'adj' --> 'adjusted peak' == peak - preceding baseline
...
for i = 1:length(sdata)
    ...
    for j = 1:length(experiment)
        ...
        first_resp_peaks        = [];
        timed_resp_peaks        = [];
        first_pre_resp_means    = [];
        timed_pre_resp_means    = [];
        first_resptraces        = [];
        timed_resptraces        = [];
        for k = 1:size(condition_mat,1)
            ...
            stim_times          = [0:1/round(this_frequency):4.6]; % recreate stimulus times from frequency
            
            resp_peaks          = [];
            pre_resp_means      = [];
            prepostsegment      = NaN(length([-prewinsize:0.001:respwinsize]),length(stim_times));
            for l = 1:length(stim_times)
                ...
                qprestimwin         = experiment(j).whiskwinedges > (stim_times(l) - prewinsize) & experiment(j).whiskwinedges < (stim_times(l));
                ...
                % get segment of whisk_profile to determine mean rate of
                % fire just before the stimulus
                pre_segment         = squeeze(mean(mean(experiment(j).whisk_profile(k,summarise_channels,qprestimwin),1),2));
                ...
                % find max firing rate
                resp_peaks(l)       = max(profile_segment);
                
                % find mean pre-stimulus baseline
                pre_resp_means(l)   = mean(pre_segment);
                
                % add trace of pre and post stimulus to matrix
                prepostsegment(1:length([pre_segment(:); profile_segment(:)]),l)  = [pre_segment(:); profile_segment(:)];
            end
            
            first_resp_peaks        = [first_resp_peaks; resp_peaks(1:4)];
            first_pre_resp_means    = [first_pre_resp_means; pre_resp_means(1:4)];
            
            % generate indices for stimuli at approximately 0, 1, 2, 3 seconds
            timed_stim_inds         = [1 find(round(stim_times*100) == 100) find(round(stim_times*100) == 200) find(round(stim_times*100) == 300) find(round(stim_times*100) == 400)];
            
            % retrieve stimulus peaks at 0:N seconds
            timed_resp_peaks        = [timed_resp_peaks; resp_peaks(timed_stim_inds)];
            timed_pre_resp_means    = [timed_pre_resp_means; pre_resp_means(timed_stim_inds)];
            
            %
            first_resptraces        = cat(3,first_resptraces, prepostsegment(:,1:4));
            timed_resptraces     	= cat(3,timed_resptraces, prepostsegment(:,timed_stim_inds));
        end
        
        freq_resp_measures(counter).first_resp_peaks    = first_resp_peaks;
        freq_resp_measures(counter).timed_resp_peaks    = timed_resp_peaks;
        freq_resp_measures(counter).first_resp_pre      = first_pre_resp_means;
        freq_resp_measures(counter).timed_resp_pre      = timed_pre_resp_means;
        freq_resp_measures(counter).adj_first_resps     = first_resp_peaks - first_pre_resp_means;
        freq_resp_measures(counter).adj_timed_resps     = timed_resp_peaks - timed_pre_resp_means;
        
        freq_resp_measures(counter).firstratio          = first_resp_peaks(:,first_vs_nr) ./ first_resp_peaks(:,1); % Express response to stimulus N as a proportion of response to stimulus 1
        freq_resp_measures(counter).timedratio          = timed_resp_peaks(:,timed_vs_nr) ./ timed_resp_peaks(:,1); % Express response to stimulus at N seconds as a proportion of response to stimulus 1 (at t=0s)
        freq_resp_measures(counter).adj_firstratio      = freq_resp_measures(counter).adj_first_resps(:,first_vs_nr) ./ freq_resp_measures(counter).adj_first_resps(:,1); % Express response to stimulus N as a proportion of response to stimulus 1
        freq_resp_measures(counter).adj_timedratio      = freq_resp_measures(counter).adj_timed_resps(:,timed_vs_nr) ./ freq_resp_measures(counter).adj_timed_resps(:,1); % Express response to stimulus at N seconds as a proportion of response to stimulus 1 (at t=0s)
        ...
        freq_resp_measures(counter).first_traces     	= first_resptraces;
        freq_resp_measures(counter).timed_traces        = timed_resptraces;
        
        %% Look at peak firing rate of first response between LED and no LED
        contrast_conds      = [1 6];
...
% first_ratio     = cell2mat({freq_resp_measures.adj_firstratio}');
% timed_ratio     = cell2mat({freq_resp_measures.adj_timedratio}');
...
if save_figs
    ...
    print(21,[experiment_plot_folder filesep 'First vs Nth response - chan ' num2str(summarise_channels(1)) '-' num2str(summarise_channels(end))],'-dpng','-r300')
    print(22,[experiment_plot_folder filesep 'First vs N seconds response - chan ' num2str(summarise_channels(1)) '-' num2str(summarise_channels(end))],'-dpng','-r300')
...
if qLEDplots
    ...
    if save_figs
        print(gcf,[experiment_plot_folder filesep 'LED traces - chan ' num2str(summarise_channels(1)) '-' num2str(summarise_channels(end))],'-dpng','-r300')
    end
