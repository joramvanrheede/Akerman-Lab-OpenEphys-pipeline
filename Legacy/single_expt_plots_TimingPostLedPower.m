...
split_conditions    = [1 6 8]; % split by these conditions, summarise over others

split_plots         = [1 6]; % [4 6] works
split_raster_plots  = [1 6 8]
condition_units     = 'units';

save_folder         = '/Users/Joram/Dropbox/Akerman Postdoc/Figures/Matlab output';
save_figs        	= false;

resp_measure        = 'area'; % 'peak' or 'area'
...
if qpsth
    ...
    set(gcf,'Position',[0 .2 1 .8])
    [axh, pos] = tight_subplot(length(unique(condition_mat(:,split_plots(1)))),length(unique(condition_mat(:,split_plots(2)))),0.02,0.02,0.02)
    for a = 1:size(split_cond_rows,1)
        ...
        %xlabel('Time (s)')
        hold on
        %set(gca,'LineWidth',2,'FontName','Garamond','FontSize',12)
