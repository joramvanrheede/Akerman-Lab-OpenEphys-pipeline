function fighandles = ephys_data_rasterplots(ephys_data,chans,split_plots,split_figures,trialrange,x_ax_lims,condition_name,condition_units)
...
for a = 1:length(chans)
    ...
    for b = 1:length(ephys_data.conditions)
        for c = 1:size(ephys_data.conditions(b).spikes,2)
            spike_cell(a,b,c) = {squeeze(ephys_data.conditions(b).spikes(a,c,:))'};
        end
    end
end
...
%% Retrieve conditions 
condition_mat           = ephys_data.condition_values;
...
for a = 1:length(split_fig_rows)
    ...
    for b = 1:length(split_plot_rows)
        ...
        if n_cols == 1
            qcond       = condition_mat(:,split_plots(1)) == this_row(1) & qselect;
        else
