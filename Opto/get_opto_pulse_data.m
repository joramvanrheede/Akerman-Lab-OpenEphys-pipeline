...
% folders. PULSE_DATA(N) is generated by PULSE_FUNCTION; see the
% PULSE_FUNCTION help for an explanation of the different fields of 
% the output struct.
% 
...
for a = 1:length(expt_folders)
    ...
    target_folder   = fullfile(data_folder,expt_folders{a});
    
    % if only a single file in folder --> 
    expt_file_list  = dir([target_folder filesep '*.mat']);
    
    if length(expt_file_list) > 1
        % Get opto pulse experiment spiking summary data for this experiment
        pulse_data(a)           = pulse_function(target_folder, resp_win, psth_bins, artifact_win);
    else
        disp(['Loading ' [target_folder filesep expt_file_list(1).name] '...'])
        load([target_folder filesep expt_file_list(1).name])
        pulse_data(a)           = pulse_function(ephys_data, resp_win, psth_bins, artifact_win);
    end
