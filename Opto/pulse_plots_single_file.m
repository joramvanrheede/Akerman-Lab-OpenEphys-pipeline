function pulse_data = pulse_plots_single_file(ephys_data, channels, resp_win, psth_bins, artifact_win)
% function pulse_data = pulse_plots_single_file(EPHYS_DATA)
% function pulse_data = pulse_plots_single_file(EPHYS_DATA, CHANNELS, RESP_WIN, PSTH_BINS, ARTIFACT_WIN)
% 
% Visualise data and extract key measures from opto 'pulse' experiment, in which
% a short optogenetic pulse is delivered each trial, at different light intensities.
% 
...
% get conditions data and give shorter variable name for easier code reading and writing
...
for b = 1:n_powers
    ...
    % spike times relative to start of trial for measuring spontaneous
    spont_spikes                = cond_data(q_power).spikes(channels,:,:);
    
    % spike times relative to opto onset
    spikes                   	= cond_data(q_power).spikes(channels,:,:) - opto_onset;
    
    q_artifact                  = spikes > artifact_win(1) & spikes < artifact_win(2);
    
    spikes(q_artifact)          = NaN;
    
    n_trials                    = cond_data(q_power).n_trials;
    
    %% Quantify spiking response
    
    % Binned spike rate
    spike_rate_by_power(:,b)                            = spike_rate_by_trial(spikes, resp_win);
    
    % Peak spike rate and time
    [spike_peak_by_power(:,b), peak_time_by_power(:,b)] = peak_ROF_by_trial(spikes, resp_win);
    
    % Spontaneous spike rate 
    spont_rate_by_power(:,b)                            = spike_rate_by_trial(spont_spikes, resp_win);
    
    % Spontaneous spike peak and time
    spont_peak_by_power(:,b)                            = peak_ROF_by_trial(spont_spikes, resp_win);
...
mean_spont_rate             = mean(spont_rate_by_power(:));
std_spont_rate              = std(spont_rate_by_power(:));
...
subplot(1,3,1)
errorbar(opto_powers,mean(corr_spike_rate_by_power),serr(corr_spike_rate_by_power),'k','LineWidth',2,'MarkerSize',30)
...
subplot(1,3,2)
errorbar(opto_powers,mean(corr_spike_peak_by_power),serr(corr_spike_peak_by_power),'k','LineWidth',2,'MarkerSize',30)
...
subplot(1,3,3)
errorbar(opto_powers,mean(peak_time_by_power),serr(peak_time_by_power),'k','LineWidth',2,'MarkerSize',30)
...
all_spont_rates                 = repmat(spont_rate_by_power(:),1,n_powers);
all_spont_peaks                 = repmat(spont_peak_by_power(:),1,n_powers);

[binned_rate_h, binned_rate_p]	= ttest2(spike_rate_by_power,all_spont_rates);
[peak_rate_h, peak_rate_p]      = ttest2(spike_peak_by_power,all_spont_peaks);


for i = 1:n_powers
    binned_rate_rank_p(i)   = ranksum(spike_rate_by_power(:,i),spont_rate_by_power(:));
    peak_rate_rank_p(i)     = ranksum(spike_peak_by_power(:,i),spont_peak_by_power(:));
end
