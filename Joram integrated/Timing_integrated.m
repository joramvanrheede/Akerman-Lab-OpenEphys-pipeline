...
L5_chans            = 5:12; % relative to L4 sink --> sink_chan + L5_chans
...
if q_reload
    ...
    for i = 1:length(group_folders)    %'F:\Sorted_synced \Pom \Timing\
    ...
    %% --> make load_multiunit_experiments that will discard LFP data?
...
for a = 1:length(merged_groups)
    ...
    for b = 1:length(merged_groups(a).prep)
        ...
        for c = 1:length(merged_groups(a).prep(b).expt_data)
            ...
            these_L5_chans      = ephys_data.max_sink_chan + L5_chans;
