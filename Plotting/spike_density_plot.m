function [image_handle, rate_data] = spike_density_plot(spike_data,split_dim,bins, make_plot)
% FUNCTION [IMAGE_HANDLE, COUNT_DATA] = SPIKE_DENSITY_PLOT(SPIKE_DATA,SPLIT_DIM,BINS,MAKE_PLOT)
% 
...
%
% MAKE_PLOT: Optional logical input ('true'/'false'). Is 'true' by default.
% If 'false', the function does not make a plot and only returns the count data
% 
% 
% IMAGE_HANDLE: A handle to the image plotted with MATLAB's IMAGESC function.
% 
% RATE_DATA: The spike rate per bin data that underlies the heatmap in the
% image. Use imagesc(RATE_DATA) to recreate the heatmap.
% 
...
n_summed    = size(spike_data,3 - split_dim); % how many [channels or trials] are summed for the counts?
bin_diff    = median(diff(bins)); % how big is a bin

% initialise count_data that will make up the heatmap image
rate_data  = zeros(n_rows, n_bins-1); 
...
for a = 1:n_rows
    ...
    % Make histogram for this row; convert to rate per trial
    rate_data(a,:)     = histcounts(trial_spikes,bins) / n_summed / bin_diff;
    
end

if make_plot
    % Display the count data as an image
    image_handle    = imagesc(rate_data);
